					...
					struct dma_block_config *block)
{
	...
	LOG_DBG("Using static descriptors");
	...
	for (i = 0; i < CONFIG_DMA_ESP32_MAX_DESCRIPTOR_NUM; ++i) {
		...
		LOG_DBG("Used %d of %d descriptors", i + 1, CONFIG_DMA_ESP32_MAX_DESCRIPTOR_NUM);
		...
		if (!block_size) {
			if (block->next_block) {
				...
				LOG_DBG("Static descriptor configuration complete, used %d descriptors", i + 1);
				break;
			}
		}
	...
	/* Check if we ran out of descriptors by completing the loop without breaking */
